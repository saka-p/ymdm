...
class YmdmCommands(Provider):
    ...
    async def discover(self) -> Hits:
        yield Hit(1.0, "Authentication", self.app.action_open_auth_menu,
                  help="Manage private playlist auth — setup, status, remove")
        yield Hit(0.9, "Developer Mode", self.app.action_open_dev_menu,
...
class YmdmApp(App):
    ...
    def action_open_dev_menu(self) -> None:
        def handle_result(choice) -> None:
            ...
            if choice == 0:
                ...
                self._set_status(f"Developer mode {state_str}.")
            elif choice == 1:
                import os
    ...
    def _do_sync_all(self) -> None:
        ...
        if all_errors:
            self.call_from_thread(self._set_status,
                f"⚠ Sync complete with {len(all_errors)} error(s) — see ~/.config/ymdm/errors.log")
        else:
    ...
    def _do_sync_selected(self, idx: int) -> None:
        ...
        if errors:
            self.call_from_thread(self._set_status,
                f"⚠ Done: {pl.name} — {len(errors)} error(s), see ~/.config/ymdm/errors.log")
        else:
    ...
    def action_rescan(self) -> None:
        ...
        cleaned = reconcile(conn)
        self._refresh_playlists()
        msg = (f"Rescan complete — {cleaned} missing track(s) removed."
               if cleaned else "Rescan complete — everything looks good.")
    ...
    def action_auth_status(self) -> None:
        if self.config.auth.enabled and self.config.auth.browser:
            self._set_status(
                f"Auth enabled — using {self.config.auth.browser} cookies. ⚠ May trigger Google security alerts.")
        else:
...
echo "Update 19 applied successfully"
