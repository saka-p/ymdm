...
_install_pkg() {
    ...
    if command -v dnf &>/dev/null; then
        ...
        echo "  ✗ Could not install $pkg — no supported package manager found."
...
if command -v ffmpeg &>/dev/null; then
    ...
    if [[ "$yn" =~ ^[Yy]$ ]]; then
        ...
        echo "  ffmpeg is required for audio conversion. Exiting."
        exit 1
    fi
fi
...
if [[ "$EXTRA" == "[desktop]" || "$EXTRA" == "[full]" ]]; then
    if python3 -c "import gi" 2>/dev/null; then
        ...
        if [[ "$yn" =~ ^[Yy]$ ]]; then
            if command -v dnf &>/dev/null; then
                sudo dnf install -y python3-gobject python3-gobject-devel libadwaita gtk4
            elif command -v apt &>/dev/null; then
                sudo apt install -y python3-gi python3-gi-cairo gir1.2-gtk-4.0 gir1.2-adw-1
            elif command -v pacman &>/dev/null; then
                sudo pacman -S --needed --noconfirm python-gobject libadwaita gtk4
            fi
        else
            echo "  pygobject is required for the GTK app. Exiting."
            exit 1
        fi
    fi
fi
